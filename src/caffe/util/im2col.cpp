...
inline void im2col_nd_core_cpu(const Dtype* data_input, const bool im2col,
  ...
  int kernel_size = 1; // 一个kernel大小的块有多大
  for (int i = 0; i < num_spatial_axes; ++i) {
  ...
  for (int c_col = 0; c_col < channels_col; ++c_col) {
    ...
    for (bool incremented = true; incremented; ) {
      ...
      // 是经过im2colnd变换之后的索引  
      int index_col = c_col;
      int index_im = c_col / kernel_size;       // index_im是原始图像中的channel  
      bool is_padding = false;
      for (int d_i = 0; d_i < num_spatial_axes; ++d_i) {
        const int d = d_iter[d_i]; // d是col_buff上的偏移，与d_pad相对(d_pad是原始图像上的偏移) 

        // 在d_pad是经过pad之后的col_buff中的坐标经过转换成原图中的坐标  
        const int d_im = d * stride[d_i] - pad[d_i] +
            d_offset[d_i] * dilation[d_i];
        is_padding |= d_im < 0 || d_im >= im_shape[d_i + 1]; // 判断经过im2colnd处理的图像上的像素是否位于输入的n维图像的上的pad的那个部分
        index_col *= col_shape[d_i + 1]; // 计算位于col_buff中的位置(就是经过im2colnd变换之后的) 
        index_col += d;
        index_im *= im_shape[d_i + 1]; // 计算位于原始图像中的位置  
        index_im += d_im;
      }
      if (im2col) {
