...
// functions.  
// 有一点需要记住的是：在模板类Layer的forward函数里面，会再次调用调用Reshape()
// 函数，也就是说，即使我们每次迭代每个minibatch里的图像（或者特征）的shape不一
// 致，也没有关系，  
// 因为在真正调用forward_cpu / forward_gpu 之前都会重新Reshape；SetUp里面的
// Reshape只是设置了初始的Top blobs 的shape  

/* 
前传调用对应的Forward_cpu或者Forward_gpu而我们知道Forward_cpu是纯虚函数，必须要
实而Forward_gpu是虚函数，如果不实现就调用 Forward_cpu函数了。前传（你必须实现自
己的Forward_cpu，实现Forward_gpu是可选的） 
*/  
template <typename Dtype>  
...
{  
    ...
    case Caffe::CPU:  
        ...
        {  
            ...
            // 这里的loss_weights我觉得应该是SetLossWeights()方法中模板函数
            // caffe_set()所初始化的loss_weight  
